...
const POIDS_TOTAL = Object.values(FAMILIES)
...
/**
 * Croissance entre deux exercices.
 * null si la comparaison n'a pas de sens.
 */
function croissance(nMoins1, n){
  ...
  /*
   * Passage par zéro :
   * −50 M € → +10 M € ne se traduit pas correctement par un taux classique.
   */
  if (a < 0 && b >= 0){
    return {
      value: null,
  ...
  /*
   * Deux exercices négatifs :
   * une perte qui diminue est considérée comme une amélioration.
   */
  if (a < 0 && b < 0){
    return {
      value: (Math.abs(a) - Math.abs(b)) / Math.abs(a),
      reason: null,
      note: 'perte_reduite',
    };
  }
...
/**
 * Ratio protégé :
 * dénominateur nul ou absent → null,
 * sauf règle explicite passée dans opts.zeroDen.
 */
...
function metriquesFondamentales(f = {}, market = {}){
  ...
  for (const [cle, serie] of [
    ['revenueGrowth', f.revenueSeries],
    ...
    /*
     * L'adaptateur garantit :
     * [0] = exercice le plus récent
     * [1] = exercice précédent
     */
    const c = croissance(serie[1], serie[0]);
  ...
  const dette = fini(f.debt);
  const cash = fini(f.cash);

  const detteNette =
    dette !== null && cash !== null
      ? dette - cash
      : null;
  ...
  if (dette !== null && dette === 0){
    /*
     * Aucune dette :
     * cashToDebt ne peut pas être calculé mathématiquement,
     * mais la situation financière correspond au maximum du barème.
     */
    out.cashToDebt = 2.5;
    notes.cashToDebt = 'dette_nulle';
  } else {
  ...
  for (const cle of [
    ...
    /*
     * Un multiple négatif ne signifie pas "bon marché".
     * On l'exclut du scoring.
     */
    if (v <= 0){
      out[cle] = null;
      notes[cle] = 'multiple_negatif';
      continue;
    }
  ...
  if (pb !== null && pb <= 0){
    notes.priceToBook = 'fonds_propres_negatifs';
  }
  ...
  out.fcfPositive =
    fini(f.freeCashFlow) === null
      ? null
      : (f.freeCashFlow > 0 ? 1 : 0);
...
/**
 * Volatilité, drawdown et performances calculés
 * exclusivement sur les clôtures réelles.
 */
...
function noteMetrique(cle, valeur){
  ...
  const scale =
    SCALES[cle]
    || (
      /^(revenue|eps|fcf)Growth$/.test(cle)
        ? SCALES.growth
        : null
    )
    || (
...
function novascore(dossier){
  ...
  /*
   * IMPORTANT :
   * score de famille = moyenne pondérée des métriques réellement disponibles.
   *
   * Une métrique absente perd réellement son poids.
   */
  const families = {};

  let poidsDisponible = 0;
  let couverture = 0;
  ...
  for (const [cle, conf] of Object.entries(FAMILIES)){
    ...
    families[cle] = {
      ...
      /*
       * weight représente ici le poids réellement disponible,
       * pas le poids théorique complet.
       */
      weight: poidsCouvert,

      baseWeight: conf.weight,

      metrics: notesMetriques,
    ...
    /*
     * La couverture réelle et le poids disponible
     * progressent uniquement avec les métriques présentes.
     */
    poidsDisponible +=
      poidsCouvert;
  ...
  /*
   * Les poids individuels totalisent exactement 100,
   * donc couverture est directement un pourcentage.
   */
  const coverage =
    Math.round(couverture);
  ...
  /*
   * Redistribution :
   * uniquement les poids réellement disponibles
   * sont redistribués pour ramener le score final sur 100.
   *
   * La couverture n'est PAS modifiée.
   */
  ...
  /*
   * Cohérence :
   * dispersion entre les familles calculées.
   *
   * Elle ne rapporte aucun point au NovaScore.
   */
  const notesFam =
  ...
  /*
   * Confiance :
   * combinaison du niveau de couverture
   * et de la cohérence.
   */
  const nivCouv =
    coverage >= 70
